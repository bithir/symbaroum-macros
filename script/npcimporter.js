...
async function extractAllData(npcData)
{
    ...
    actor = await Actor.update(newValues);    
    ...
    if( abilitilist !== null) {
        ...
        await abilitilist.forEach(async element => { 
            ...
            if( tmpdata != null && tmpdata.length == 3)
            {
                ...
                {
                    ability = ability[0];
                    console.log("Ability for "+tmpdata[1]+" at "+tmpdata[2]+" is "+JSON.stringify(ability));
                    // console.log(JSON.stringify(ability)); 
                    let newData = {  }; // , ability, {insertKeys: true, insertValues: false, overwrite: false});
                    // console.log("newData for "+tmpdata[1]+" is "+JSON.stringify(newData));
                    if(tmpdata[2] === "master") {                    
                        higherLevel = true;
                        newData["data.master.isActive"] = true;
                    }                
                    if(tmpdata[2] === "adept" || higherLevel) {                
                        higherLevel = true;
                        newData["data.adept.isActive"] = true;                    
                    }    
                    if(tmpdata[2] === "novice" || higherLevel) {                              
                        newData["data.novice.isActive"] = true;
                        // setProperty(ability, "data.novice.isActive",true);                        
                    }
                    // let updateObj = await actor.createOwnedItem(ability);
                    // setProperty(newData, "effects",[]);
                    // console.log("newData for "+tmpdata[1]+" is "+JSON.stringify(newData));
                    let updateObj = await actor.createOwnedItem(ability);
                    console.log("updateObj "+JSON.stringify(updateObj));
                    // await actor.createOwnedItem(ability);
                    newData._id = updateObj._id;
                    let finalObj = await actor.updateOwnedItem(newData); //, {overwrite: true, recursive: true, insertValues: false});
                    console.log("finalItem "+JSON.stringify(finalObj));
                    // await actor.update({ _id: actor._id}, null);
                    // await actor.createOwnedItem(ability);
                    // Item.createOwned(finalItem, actor);
                    /* let updateObj = await actor.createOwnedItem(ability);
                    
                    console.log("Update object "+JSON.stringify(updateObj));
                    newData._id = updateObj._id;
                    // updateObj = mergeObject(updateObj, newData, {overwrite: true, recursive: true});
                    let finalObj = await actor.updateOwnedItem(updateObj, newData); //, {overwrite: true, recursive: true, insertValues: false});
       ...
       // console.log(JSON.stringify(abilitilist));
