...
async function rollAttribute(actorids, attribute)
{
    ...
    let updates = actorids.map(a => {
        ...
        let success = (aexp.data.data.attributes[attribute].total >= roll.total) ? "succeeded":"failed";
        html += `${aexp.name} ${success} their ${attribute} roll (${aexp.data.data.attributes[attribute].total}←${roll.total} - margin ${aexp.data.data.attributes[attribute].total - roll.total })<br>`
    });
    ChatMessage.create({
        speaker: ChatMessage.getSpeaker({alias: "Hidden roll"}),
        whisper: [game.user],
        content: html
    });
}
